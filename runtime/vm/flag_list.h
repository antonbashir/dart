...
#define FLAG_LIST(P, R, C, D)                                                                                                                             \
    ...
    P(abort_on_oom, bool, false, "Abort if memory allocation fails - use only with --old-gen-heap-size")                                                  \
    P(add_readonly_data_symbols, bool, false, "Add static symbols for objects in snapshot read-only data")                                                \
    P(background_compilation, bool, true, "Run optimizing compilation in background")                                                                     \
    P(check_token_positions, bool, false, "Check validity of token positions while compiling flow graphs")                                                \
    P(collect_dynamic_function_names, bool, true, "Collects all dynamic function names to identify unique targets")                                       \
    ...
    C(deoptimize_alot, false, false, bool, false, "Deoptimizes we are about to return to Dart code from native entries.")                                 \
    C(deoptimize_every, 0, 0, int, 0, "Deoptimize on every N stack overflow checks")                                                                      \
    P(deoptimize_on_runtime_call_every, int, 0, "Deoptimize functions on every runtime call.")                                                            \
    P(dontneed_on_sweep, bool, false, "madvise(DONTNEED) free areas in partially used heap regions")                                                      \
    R(dump_megamorphic_stats, false, bool, false, "Dump megamorphic cache statistics")                                                                    \
    ...
    P(enable_mirrors, bool, true, "Disable to make importing dart:mirrors an error.")                                                                     \
    P(enable_ffi, bool, true, "Disable to make importing dart:ffi an error.")                                                                             \
    P(force_clone_compiler_objects, bool, false, "Force cloning of objects needed in compiler (ICData and Field).")                                       \
    P(guess_icdata_cid, bool, true, "Artificially create type feedback for arithmetic etc. operations")                                                   \
    P(huge_method_cutoff_in_ast_nodes, int, 10000,                                                                                                        \
      "Huge method cutoff in AST nodes: Disables optimizations for huge "                                                                                 \
      "methods.")                                                                                                                                         \
    P(idle_timeout_micros, int, 61 * kMicrosecondsPerSecond, "Consider thread pool isolates for idle tasks after this long.")                             \
    P(idle_duration_micros, int, kMaxInt32, "Allow idle tasks to run for this long.")                                                                     \
    P(interpret_irregexp, bool, false, "Use irregexp bytecode interpreter")                                                                               \
    P(link_natives_lazily, bool, false, "Link native calls lazily")                                                                                       \
    R(log_marker_tasks, false, bool, false, "Log debugging information for old gen GC marking tasks.")                                                    \
    ...
    P(hash_map_probes_limit, int, kMaxInt32, "Limit number of probes while doing lookups in hash maps.")                                                  \
    P(max_polymorphic_checks, int, 4, "Maximum number of polymorphic check, otherwise it is megamorphic.")                                                \
    P(max_equality_polymorphic_checks, int, 32, "Maximum number of polymorphic checks in equality operator,")                                             \
    P(new_gen_semi_max_size, int, kDefaultNewGenSemiMaxSize, "Max size of new gen semi space in MB")                                                      \
    P(new_gen_semi_initial_size, int, (kWordSize <= 4) ? 1 : 2, "Initial size of new gen semi space in MB")                                               \
    P(optimization_counter_threshold, int, kDefaultOptimizationCounterThreshold, "Function's usage-counter value before it is optimized, -1 means never") \
    P(optimization_level, int, 2, "Optimization level: 1 (favor size), 2 (default), 3 (favor speed)")                                                     \
    ...
    R(pause_isolates_on_start, false, bool, false, "Pause isolates before starting.")                                                                     \
    R(pause_isolates_on_exit, false, bool, false, "Pause isolates exiting.")                                                                              \
    R(pause_isolates_on_unhandled_exceptions, false, bool, false, "Pause isolates on unhandled exceptions.")                                              \
    P(polymorphic_with_deopt, bool, true, "Polymorphic calls with deoptimization / megamorphic call")                                                     \
    ...
    P(print_snapshot_sizes_verbose, bool, false, "Print cluster sizes of generated snapshots.")                                                           \
    R(print_ssa_liveranges, false, bool, false, "Print live ranges after allocation.")                                                                    \
    R(print_stacktrace_at_api_error, false, bool, false, "Attempt to print a native stack trace when an API error is created.")                           \
    D(print_variable_descriptors, bool, false, "Print variable descriptors in disassembly.")                                                              \
    R(profiler, false, bool, false, "Enable the profiler.")                                                                                               \
    R(profiler_native_memory, false, bool, false, "Enable native memory statistic collection.")                                                           \
    P(reorder_basic_blocks, bool, true, "Reorder basic blocks")                                                                                           \
    C(stress_async_stacks, false, false, bool, false, "Stress test async stack traces")                                                                   \
    ...
    P(retain_code_objects, bool, true,                                                                                                                    \
      ...
      "needed in the precompiled runtime.")                                                                                                               \
    P(show_invisible_frames, bool, false, "Show invisible frames in stack traces.")                                                                       \
    P(target_unknown_cpu, bool, false, "Generate code for a generic CPU, unknown at compile time")                                                        \
    ...
    D(trace_ic_miss_in_optimized, bool, false, "Trace IC miss in optimized code")                                                                         \
    C(trace_irregexp, false, false, bool, false, "Trace irregexps.")                                                                                      \
    D(trace_intrinsified_natives, bool, false, "Report if any of the intrinsified natives are called")                                                    \
    ...
    R(trace_ssa_allocator, false, bool, false, "Trace register allocation over SSA.")                                                                     \
    P(trace_strong_mode_types, bool, false, "Trace optimizations based on strong mode types.")                                                            \
    D(trace_type_checks, bool, false, "Trace runtime type checks.")                                                                                       \
    D(trace_type_checks_verbose, bool, false, "Enable verbose trace of runtime type checks.")                                                             \
    D(trace_patching, bool, false, "Trace patching of code.")                                                                                             \
    D(trace_optimized_ic_calls, bool, false, "Trace IC calls in optimized code.")                                                                         \
    D(trace_zones, bool, false, "Traces allocation sizes in the zone.")                                                                                   \
    P(truncating_left_shift, bool, true, "Optimize left shift to truncate if possible")                                                                   \
    P(use_compactor, bool, false, "Compact the heap during old-space GC.")                                                                                \
    P(use_incremental_compactor, bool, true, "Compact the heap during old-space GC.")                                                                     \
    P(use_cha_deopt, bool, true, "Use class hierarchy analysis even if it can cause deoptimization.")                                                     \
    ...
    R(verify_after_gc, false, bool, false, "Enables heap verification after GC.")                                                                         \
    R(verify_before_gc, false, bool, false, "Enables heap verification before GC.")                                                                       \
    R(verify_store_buffer, false, bool, false, "Enables store buffer verification before and after scavenges.")                                           \
    R(verify_after_marking, false, bool, false, "Enables heap verification after marking.")                                                               \
    P(enable_slow_path_sharing, bool, true, "Enable sharing of slow-path code.")                                                                          \
    P(shared_slow_path_triggers_gc, bool, false, "TESTING: slow-path triggers a GC.")                                                                     \
    P(enable_multiple_entrypoints, bool, true, "Enable multiple entrypoints per-function and related optimizations.")                                     \
    P(enable_testing_pragmas, bool, false, "Enable magical pragmas for testing purposes. Use at your own risk!")                                          \
    R(eliminate_type_checks, true, bool, true, "Eliminate type checks when allowed by static type analysis.")                                             \
    D(support_rr, bool, false, "Support running within RR.")                                                                                              \
    P(coroutines_array_initial_size, int, 64, "All coroutines array initial size.")                                                                       \
